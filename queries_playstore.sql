...
SELECT app_name, reviews, rating FROM analytics WHERE rating >= 4.8 ORDER BY rating,reviews desc;
...
SELECT app_name, reviews, price FROM analytics WHERE price BETWEEN .10 AND 1.00;
...
SELECT COUNT(reviews) AS count_reviews FROM analytics;

--14. Find all the categories that have more than 300 apps in them.
SELECT category, COUNT(app_name) FROM analytics GROUP BY category HAVING COUNT(*)>300 ORDER BY COUNT(*) desc; 
...
SELECT app_name, reviews, min_installs, min_installs/reviews AS porportion FROM analytics WHERE min_installs > 100000 ORDER BY porportion DESC LIMIT 1;
